...
/*
Accept connection, receive request from client and
fill into var req. Return 1 if fail to accept or receive
return 0 if succeed
*/
int acceptReq(char* req, int reqSize) {
	...
	if (connSock == INVALID_SOCKET) {
		...
		iRetVal = recv_s(connSock, req, reqSize);
		if (iRetVal == SOCKET_ERROR) {
			printf("Error code %d.\n", WSAGetLastError());
			return 1;
		}
		...
		printf("Receive from client [%s:%d] %s\n",
			inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port), req);
...
int sendRes(char* res, int resultLen) {
	int iRetVal;
	// Send message
	iRetVal = send_s(connSock, res, resultLen);
...
int readUserF(std::list<Account>& accList) {
	...
	while (!feof(f)) {
		Account acc;
		fgets(filebuff, FILEBUFF_SIZE, f);
...
int writeUserF(std::list<Account> accList) {
	...
	endIt--;
	for (; it != endIt; it++) {
		snprintf(filebuff, FILEBUFF_SIZE, "%s %s %d\n",
