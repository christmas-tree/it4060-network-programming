...
int recvPacket(SOCKET sock, short* opCode, short* len, char* payload, short payloadMaxLen) {
	...
	if (iRetVal == SOCKET_ERROR) {
		printf("[!] Error %d! Cannot receive from client!\n", WSAGetLastError());
		// If error is WSAEWOULDBLOCK, wait 50ms and try again
		if (WSAGetLastError() == WSAEWOULDBLOCK) {
			Sleep(50);
			iRetVal = send(sock, buff, 3, 0);
...
*/
int sendFile(SOCKET sock, char* fileName, char* digest) {
	char buff[FILEBUFF_SIZE];
	FILE* f;
	short iRetVal;
	short readSize;
	...
	while (1) {
		...
		printf("Sent file to client %d, part %d\n", sock, ++filePart);
		if (readSize == 0) break;
	}
	fclose(f);

	// Send MD5 code
	iRetVal = sendPacket(sock, OP_FILE, DIGEST_SIZE, digest);
	if (iRetVal == SOCKET_ERROR) {
		printf("Error code %d while sending digest.\n", WSAGetLastError());
		return 1;
	}
