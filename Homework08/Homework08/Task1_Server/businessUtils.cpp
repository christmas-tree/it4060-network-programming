...
void run() {
	...
	while (1) {
		...
		if (sockEvent.lNetworkEvents & FD_READ) {
			...
			iRetVal = recvReq(socks[index], buff, BUFF_LEN);
			if (iRetVal == 0) {
				parseRequest(socks[index], buff);
				iRetVal = sendRes(socks[index], buff, strlen(buff));
			}

			if (iRetVal != 0) {
				disconnect(socks[index]);
		...
		if (sockEvent.lNetworkEvents & FD_CLOSE) {
			if (sockEvent.iErrorCode[FD_CLOSE_BIT] != 0) {
				printf("Error %d! FD_CLOSE failed.\n", sockEvent.iErrorCode[FD_CLOSE_BIT]);
				continue;
			}
