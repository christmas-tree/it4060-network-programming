	...
	while (true) {
		...
		if (perIoData->operation == RECEIVE) {
			...
			if (perIoData->recvBytes < perIoData->intendedLength) {
				...
				// Add a null character to end string
				perIoData->buffer[perIoData->intendedLength] = 0;

				// Push a perQueueItem to businessQueue for business threads to process
				perQueueItem.perHandleData = perHandleData;
				...
				businessQueue.push(perQueueItem);
...
unsigned __stdcall serverBusinessThread(LPVOID none)
{
	...
	while (true) {
		...
		payloadLength = strlen(perQueueItem.perIoData->buffer+4);
		...
		perQueueItem.perIoData->dataBuff.len = perQueueItem.perIoData->intendedLength;
